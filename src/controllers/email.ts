...
class Email {
	...
	protected static convertEmail(email: IEmail, env: Env): IMCEmail {
		...
		if (DKIM_SELECTOR && DKIM_SELECTOR && DKIM_PRIVATE_KEY) {
			personalizations.push({ to: toContacts, dkim_domain: DKIM_DOMAIN, dkim_selector: DKIM_SELECTOR, dkim_private_key: DKIM_PRIVATE_KEY});
		} else {
